...
AVIATION_MCP_COMMAND = shutil.which("aviationstack-mcp")
UVX_COMMAND = shutil.which("uvx") or "uvx"
AVIATION_COMMAND = AVIATION_MCP_COMMAND or UVX_COMMAND
AVIATION_ARGS = [] if AVIATION_MCP_COMMAND else ["aviationstack-mcp==1.6.0"]
...
async def _get_server_tool(
    ...
    if server_name == "tavily":
        ...
        if shutil.which(AVIATION_COMMAND) is None:
            raise RuntimeError(
                "The AviationStack MCP executable was not found. Install "
                "aviationstack-mcp==1.6.0 or install uv and run it with uvx."
            )
